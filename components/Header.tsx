...
const Header: React.FC<HeaderProps> = ({ isDarkMode, toggleDarkMode }) => {
  ...
  const navLinks = [
    ...
    { name: 'الحياة الطلابية', href: '/visitor-login', isLink: true },
    ...
    <header className="bg-white dark:bg-gray-800 shadow-md sticky top-0 z-50 dark-transition animate-slideInFromTop">
        ...
        <div className="container mx-auto px-4 py-1.5 flex justify-between items-center text-xs md:text-sm text-gray-600 dark:text-gray-300">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            {isAuthenticated ? (
              ...
              <>
                <Link to="/visitor-login" className="text-xs md:text-sm font-bold text-gray-700 dark:text-gray-200 hover:text-red-600 dark:hover:text-red-500 hover-scale transition-all duration-300">تسجيل الدخول</Link>
                <Link to="/register" className="text-xs md:text-sm font-bold bg-red-600 text-white px-2 md:px-3 py-1 md:py-1.5 rounded-md hover:bg-red-700 hover-lift transition-all duration-300">إنشاء حساب</Link>
              </>
            )}
      ...
      <div className="container mx-auto px-4 py-2 md:py-3 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2 hover:opacity-80 hover-scale transition-all duration-300">
          <h1 className="text-lg md:text-2xl font-black text-red-600">مدرسة الاحايوه شرق</h1>
